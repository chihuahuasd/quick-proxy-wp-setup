...
# Create comprehensive wp-config additions
cat > /tmp/wp-proxy-config.php << EOF
// === DEDICATED PROXY CONFIGURATION ===
// Handle missing HTTP_HOST header
if (!isset($_SERVER['HTTP_HOST']) || empty($_SERVER['HTTP_HOST'])) {
    $_SERVER['HTTP_HOST'] = $DOMAIN;
}

// Handle reverse proxy headers safely
...
// Safe IP forwarding handling
if (isset($_SERVER['HTTP_X_FORWARDED_FOR']) && is_string($_SERVER['HTTP_X_FORWARDED_FOR']) && !empty($_SERVER['HTTP_X_FORWARDED_FOR'])) {
    $forwarded_ips = explode(',', $_SERVER['HTTP_X_FORWARDED_FOR']);
    if (is_array($forwarded_ips) && isset($forwarded_ips[0])) {
...
if [ -f "$WP_CONFIG" ]; then
    sudo sed -i "/\/\* That's all, stop editing! Happy publishing\. \*\//r /tmp/wp-proxy-config.php" $WP_CONFIG
    echo "✓ WordPress configuration updated"
else
    echo "⚠️ wp-config.php not found - you'll need to add the configuration manually"
fi

# Clean up temp file
rm /tmp/wp-proxy-config.php
